...
public static class TypeBuilderHelper
{
    ...
    public static Type BuildType(TypeConfig typeConfig)
    {
        ...
        typeConfig.Attributes.ForEach(aAttribute => { tb.SetCustomAttribute(GetCustomAttributeBuilder(aAttribute)); });
    ...
    private static CustomAttributeBuilder GetCustomAttributeBuilder(AttributeConfig attributeConfig)
    {
        ...
        var attributeConstructor =
            attributeType.GetConstructor(attributeConfig.ConstructorArgs.Select(x => x.GetType()).ToArray());
...
public class AttributeConfig
{
    ...
    /// 初始化属性
    /// </summary>
    public List<(string PropertyName, object Value)> Properties { get; set; } =
        new List<(string PropertyName, object Value)>();
}
