...
using Bear.Core.Common.MultiLanguage.Resources;
...
namespace Bear.Core.TaskService.service;
...
public class SchedulerCenterService : ISchedulerCenterService
{
    ...
    /// </summary>
    /// <returns></returns>
    public async Task<int> DeleteScheduleJobAsync(QuartzNet taskQuartz)
    {
        try
        {
            JobKey jobKey = new JobKey(taskQuartz.Id.ToString(), taskQuartz.TaskGroup);
    ...
    /// <returns></returns>
    public async Task<string> GetTriggerStatus(QuartzNetVo taskQuartzVo)
    {
        string triggerStatus = Localizer.R("Enum.Trigger.NotRunning");
        JobKey jobKey = new JobKey(taskQuartzVo.Id.ToString(), taskQuartzVo.TaskGroup);
        ...
        foreach (var trigger in triggers)
        {
            if (((JobDetailImpl)job).Name == ((AbstractTrigger)trigger).Name)
            {
                ...
                {
                    TriggerState.Blocked => Localizer.R("Enum.Trigger.Blocked"),
                    TriggerState.Complete => Localizer.R("Enum.Trigger.Complete"),
                    TriggerState.Error => Localizer.R("Enum.Trigger.Error"),
                    TriggerState.None => Localizer.R("Enum.Trigger.None"),
                    TriggerState.Normal => Localizer.R("Enum.Trigger.Normal"),
                    TriggerState.Paused => Localizer.R("Enum.Trigger.Paused"),
                    _ => triggerStatus
                };
            }
        }
